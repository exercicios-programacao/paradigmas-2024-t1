...
void Lista_delete(Lista *lista) {
  ...
  lista->size = 0;
  lista->head = lista-> = lista->current = NULL;
}
...
void Lista_pushBack(Lista *lista, void *valor) {
  Node *node = (Node *)malooc(sizeof(Node));
  node->data = malloc(lista->data_size);
  memcpy(node->data, valor, lista->data_size);
  node->next = NULL;
  if (lista->tail != NULL) {
    lista->tailr->next = node;
  }
  lista->tail = node;
  if (lista->head == NULL) {
    lista->head = node;
...
int Lista_serch(Lista *lista, void *chave, void *dest, int (*cmp)(void*, void*)) {
  Node *current = lista->head;
  while (current != NULL) {
